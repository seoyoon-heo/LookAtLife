...
interface Plate {
    ...
    options: string[];
    answer: string;
    weight: 'protan' | 'deutan' | 'tritan' | 'normal';
    bg: string[];
    fg: string[];
}
...
function ColorTest({ onResult }: ColorTestProps) {
    ...
    function randColor(colors: string[]): string {
    ...
    useEffect(() => {
        ...
        const raf = requestAnimationFrame(() => {
            if (canvasRef.current) drawPlate(canvasRef.current, PLATES[step]);
        });
    ...
    const analyze = (ans: string[]) => {
        ...
        ans.forEach((a, i) => {
            const p = PLATES[i];
            if (a !== p.answer && p.weight !== 'normal') miss[p.weight]++;
        });
    ...
    return (
        <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                <div style={{ flex: 1, height: '5px', backgroundColor: theme.colors.border, borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{ width: `${progress}%`, height: '100%', backgroundColor: theme.colors.primary, borderRadius: '4px', transition: 'width 0.35s ease' }} />
                </div>
                <span style={{ fontSize: '12px', color: theme.colors.textSub, whiteSpace: 'nowrap' }}>
                    {step + 1} / {PLATES.length}
                </span>
            </div>
            <div style={{ display: 'inline-block', padding: '3px 12px', backgroundColor: theme.colors.primaryLight, color: theme.colors.primary, borderRadius: '20px', fontSize: '11px', fontWeight: '700', marginBottom: '12px' }}>
                {plate.category}
            </div>
            <p style={{ fontSize: '15px', fontWeight: '600', color: theme.colors.text, marginBottom: '18px' }}>
                {plate.question}
            </p>
            <div style={{ position: 'relative', display: 'inline-block', marginBottom: '20px' }}>
                ...
                <canvas ref={canvasRef} width={400} height={400} style={{ width: '220px', height: '220px', borderRadius: '50%', boxShadow: '0 6px 24px rgba(0,0,0,0.18)', display: 'block', imageRendering: 'auto' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {plate.options.map(opt => (
                    <button key={opt} onClick={() => handleAnswer(opt)} style={{ width: '100%', padding: '13px 18px', border: `1.5px solid ${theme.colors.border}`, borderRadius: theme.radius.md, backgroundColor: theme.colors.white, fontSize: '15px', fontWeight: '500', color: theme.colors.text, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '12px', fontFamily: 'inherit', transition: 'border-color 0.15s' }}>
                        <div style={{ width: '20px', height: '20px', borderRadius: '50%', border: `2px solid ${theme.colors.border}`, flexShrink: 0 }} />
                        {opt}
                    </button>
                ))}
            </div>
...
const TPO_COLOR: Record<string, string> = {
...
interface HistoryRecord {
    ...
    outfitInfos?: Record<number, OutfitInfo>;
    acceptedOutfitIndex?: number;
}

interface DaltonizeResult {
    original: string;
    simulated: string;
    corrected: string;
}
...
function MyPage() {
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onloadend = async () => {
            setDaltonizing(true);
            try {
                const res = await colorAssistantAPI.daltonize(reader.result as string, profile?.colorType || '');
    ...
    const handleChangeAccept = async (recId: number, outfitIndex: number, info?: OutfitInfo) => {
        try {
    ...
    return (
        <div style={S.page}>
            ...
            <div style={S.container}>
                ...
                <div style={S.tabRow}>
                    {TABS.map(t => (
                        <button key={t} style={{ ...S.tabBtn, borderBottom: tab === t ? `2px solid ${theme.colors.primary}` : '2px solid transparent', color: tab === t ? theme.colors.primary : '#888', fontWeight: tab === t ? '700' : '400' }} onClick={() => setTab(t)}>{t}</button>
                    ))}
                </div>
                    ...
                    <div style={S.section}>
                        <div style={S.sectionHeader}>
                            ...
                            {!editMode ? (
                                ...
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    <button style={S.saveBtn} onClick={handleSave} disabled={loading}>{loading ? '저장 중...' : '저장'}</button>
                        ...
                        {!editMode ? (
                            <div>
                                ...
                                <div style={S.infoRow}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                        <span style={S.infoValue}>{COLOR_TYPE_LABELS[profile.colorType] || '미설정'}</span>
                                        <button style={CS.testTriggerBtn} onClick={() => { setShowColorTest(true); setTestActive(true); setTestResult(null); }}>
                                ...
                                {showColorTest && (
                                    <div style={CS.inlineTestBox}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                            ...
                                            <button style={CS.closeTestBtn} onClick={() => { setShowColorTest(false); setTestActive(false); setTestResult(null); }}>✕</button>
                                        </div>
                                        <p style={CS.inlineTestDesc}>의료 진단이 아닌 보조적 목적의 간이 테스트입니다. (총 8문항)</p>
                                        {testActive && !testResult && (
                                            <ColorTest onResult={(r) => { setTestResult(r); setTestActive(false); }} />
                                        )}
                                        {testResult && (
                                            <div style={CS.resultBox}>
                                                ...
                                                <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                                                    <button style={CS.saveColorBtn} onClick={() => handleSaveColorType(testResult)}>이 결과로 저장</button>
                                                    <button style={CS.retryBtn} onClick={() => { setTestResult(null); setTestActive(true); }}>다시 테스트</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div>
                                ...
                                <input style={S.editInput} value={editForm.nickname} onChange={e => setEditForm({ ...editForm, nickname: e.target.value })} maxLength={20} />
                                <label style={S.editLabel}>연령대</label>
                                <select style={S.editInput} value={editForm.ageGroup} onChange={e => setEditForm({ ...editForm, ageGroup: e.target.value })}>
                                    {AGE_GROUPS.map(a => <option key={a} value={a}>{a}</option>)}
                                </select>
                                <label style={S.editLabel}>성별</label>
                                <select style={S.editInput} value={editForm.gender} onChange={e => setEditForm({ ...editForm, gender: e.target.value })}>
                                ...
                                <div style={S.styleGrid}>
                                    {STYLES.map(s => (
                                        <button key={s} type="button" style={{ ...S.styleToggle, backgroundColor: editForm.styles.includes(s) ? theme.colors.primary : '#f0f0f0', color: editForm.styles.includes(s) ? 'white' : '#333' }} onClick={() => toggleStyle(s)}>
                                            {STYLE_LABELS[s]}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {history.length === 0 ? (
                            ...
                            history.map((rec, i) => (
                                <div key={rec.recId || i} style={S.histCard}>
                                    <div style={{ ...S.histHeader, background: `linear-gradient(135deg, ${TPO_COLOR[rec.tpo] || theme.colors.primary}DD, ${TPO_COLOR[rec.tpo] || theme.colors.primary}99)` }}>
                                        <div style={S.histHeaderTop}>
                                            <span style={S.histTpoPill}>{rec.tpo}</span>
                                            {rec.retryCount && rec.retryCount > 0 && <span style={S.histRetryPill}>🔄 재추천 {rec.retryCount}회</span>}
                                            <span style={S.histCreatedAt}>{formatDate(rec.createdAt)}</span>
                                        </div>
                                        <div style={S.histHeaderBottom}>
                                            {rec.outfitDate ? (
                                                <span style={S.histDateBig}>📅 {formatOutfitDate(rec.outfitDate)} 코디</span>
                                            ) : (
                                                <span style={S.histDateBig}>코디 추천</span>
                                            )}
                                            {rec.temperature && <span style={S.histWeatherPill}>🌡 {rec.temperature}°C · {rec.weatherCondition}</span>}
                                        </div>
                                    </div>
                                    <div style={S.histBody}>
                                        {rec.description && <p style={S.histGlobalDesc}>"{rec.description}"</p>}
                                        {rec.allOutfitGroups && Object.keys(rec.allOutfitGroups).length > 0 ? (
                                            Object.entries(rec.allOutfitGroups).map(([idxStr, items]) => {
                                                ...
                                                return (
                                                    <div key={idx} style={{ ...S.histOutfitBlock, background: isAccepted ? theme.colors.primaryLight : '#F7F9FB', borderLeft: isAccepted ? `3px solid ${theme.colors.primary}` : '3px solid #E0E6ED' }}>
                                                        <div style={S.histBlockHead}>
                                                            <div style={S.histBlockHeadLeft}>
                                                                <span style={{ ...S.histOutfitNum, background: isAccepted ? theme.colors.primary : '#B0BEC5' }}>{idx + 1}</span>
                                                                {info?.style && <span style={{ ...S.histStyleTag, color: isAccepted ? theme.colors.primary : '#607D8B', background: isAccepted ? 'white' : '#EEF2F5', border: isAccepted ? `1px solid ${theme.colors.primary}44` : '1px solid #DDE3E9' }}>{info.style}</span>}
                                                            </div>
                                                            {isAccepted && <span style={S.histAcceptedBadge}>✓ 선택됨</span>}
                                                        </div>
                                                        {items.length > 0 ? (
                                                            <div style={S.histStrip}>
                                                                {items.map((item, ii) => (
                                                                    <div key={ii} style={S.histStripItem}>
                                                                        {item.imageUrl ? (
                                                                            <img src={item.imageUrl} alt={item.type} style={S.histStripImg} />
                                                                        ) : (
                                                                            <div style={S.histStripImgEmpty}>👗</div>
                                                                        )}
                                                                        <span style={{ ...S.histCatLabel, background: CAT_COLOR[item.category || ''] || theme.colors.primary }}>{item.category}</span>
                                                        ...
                                                        {!isAccepted && (
                                                            <button style={S.histChangeCta} onClick={() => handleChangeAccept(rec.recId, idx, info)}>이 코디로 변경하기</button>
                                                        )}
                                                    </div>
                                                );
                                            })
                    ...
                    <div>
                        <div style={S.section}>
                            ...
                            <div style={CS.typeBox}>
                                ...
                                <p style={CS.typeValue}>{COLOR_TYPE_LABELS[profile.colorType] || '미설정'}</p>
                                {!profile.colorType && <p style={{ fontSize: '12px', color: '#aaa', margin: '4px 0 0' }}>프로필 탭에서 색각 유형 테스트를 진행해주세요</p>}
                            </div>
                                ...
                                (profile.styles || []).map(styleKey => {
                                    ...
                                    return (
                                        <div key={styleKey} style={CS.paletteSection}>
                                            <p style={CS.paletteSectionTitle}>{STYLE_LABELS[styleKey]} 스타일 추천 색상</p>
                                                ...
                                                <div key={pi} style={CS.paletteCard}>
                                                    <div style={CS.paletteTop}>
                                                        ...
                                                        <div style={CS.colorDots}>
                                                            {p.colors.map((c, ci) => (
                                                                <div key={ci} style={{ ...CS.colorDot, backgroundColor: c, border: ['#ffffff','#f5f5f5','#F5DEB3','#FFFACD'].includes(c) ? '1px solid #e0e0e0' : 'none' }} />
                                                            ))}
                                                        </div>
                                                    </div>
                            ...
                            {profile.colorType && profile.colorType !== 'normal' && COLOR_TYPE_WARN[profile.colorType] && (
                                <div style={{ marginTop: '24px' }}>
                                    <h3 style={{ ...CS.subTitle, color: '#E74C3C' }}>⚠️ {COLOR_TYPE_WARN[profile.colorType]!.label}</h3>
                                    <p style={CS.desc}>색각 유형에 따라 아래 색상 조합은 코디 시 주의해주세요.</p>
                                            ...
                                            <div style={CS.paletteTop}>
                                                ...
                                                <div style={CS.colorDots}>
                                                    {item.colors.map((c, ci) => (
                                                        <div key={ci} style={{ ...CS.colorDot, backgroundColor: c, border: '1px solid rgba(0,0,0,0.1)' }} />
                                                    ))}
                                                </div>
                                            </div>
                        ...
                        {profile.colorType && profile.colorType !== 'normal' && (
                            <div style={{ ...S.section, marginTop: '16px' }}>
                                ...
                                <p style={CS.desc}>의류 이미지를 업로드하면 색각 이상 시뮬레이션과 보정 결과를 비교할 수 있습니다.</p>
                                <button style={CS.uploadBtn} onClick={() => fileInputRef.current?.click()}>이미지 업로드</button>
                                <input ref={fileInputRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handleImageUpload} />
                                {daltonizing && <p style={CS.loadingText}>보정 처리 중...</p>}
                                {daltonizeResult && (
                                    <>
                                        <button style={{ ...CS.uploadBtn, backgroundColor: theme.colors.white, color: theme.colors.primary, border: `1px solid ${theme.colors.primary}`, marginBottom: '12px', marginLeft: '6px' }} onClick={() => fileInputRef.current?.click()}>
                                            다른 사진 보정하기
                                        </button>
...
const S: Record<string, React.CSSProperties> = {
    ...
    profileHeader: { display: 'flex', alignItems: 'center', gap: '16px', backgroundColor: theme.colors.white, borderRadius: theme.radius.xl, padding: '20px', marginBottom: '16px', boxShadow: theme.colors.cardShadow },
    avatar: { width: '64px', height: '64px', borderRadius: '50%', backgroundColor: theme.colors.primary, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: '700', flexShrink: 0 },
    ...
    logoutBtn: { padding: '8px 16px', backgroundColor: 'white', color: theme.colors.danger, border: `1px solid ${theme.colors.danger}`, borderRadius: theme.radius.full, fontSize: '13px', cursor: 'pointer', fontWeight: '500', alignSelf: 'flex-start' },
    tabRow: { display: 'flex', marginBottom: '16px', backgroundColor: theme.colors.white, borderRadius: theme.radius.xl, padding: '4px', boxShadow: theme.colors.cardShadow },
    tabBtn: { flex: 1, padding: '11px', background: 'none', border: 'none', fontSize: '13px', cursor: 'pointer', borderRadius: theme.radius.lg },
    section: { backgroundColor: theme.colors.white, borderRadius: theme.radius.xl, padding: '20px', boxShadow: theme.colors.cardShadow },
    sectionHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' },
    sectionTitle: { fontSize: '17px', fontWeight: '700', color: theme.colors.text, margin: '0 0 16px' },
    editBtn: { padding: '7px 16px', backgroundColor: theme.colors.primaryLight, color: theme.colors.primary, border: 'none', borderRadius: theme.radius.full, fontSize: '13px', cursor: 'pointer', fontWeight: '500' },
    saveBtn: { padding: '7px 16px', backgroundColor: theme.colors.primary, color: 'white', border: 'none', borderRadius: theme.radius.full, fontSize: '13px', cursor: 'pointer', fontWeight: '600' },
    cancelBtn: { padding: '7px 16px', backgroundColor: theme.colors.background, color: theme.colors.textSub, border: 'none', borderRadius: theme.radius.full, fontSize: '13px', cursor: 'pointer' },
    infoRow: { display: 'flex', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${theme.colors.border}` },
    ...
    styleChip: { padding: '4px 12px', backgroundColor: theme.colors.primaryLight, borderRadius: theme.radius.full, fontSize: '12px', color: theme.colors.primary, fontWeight: '500' },
    editLabel: { fontSize: '12px', color: theme.colors.textSub, display: 'block', marginBottom: '6px', marginTop: '14px' },
    editInput: { width: '100%', padding: '10px 12px', borderRadius: theme.radius.md, border: `1px solid ${theme.colors.border}`, fontSize: '14px', boxSizing: 'border-box' },
    ...
    histCard: { background: 'white', borderRadius: '20px', marginBottom: '16px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', border: '1px solid rgba(0,0,0,0.04)' },
    histHeader: { padding: '16px 18px 14px' },
    histHeaderTop: { display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '10px', flexWrap: 'wrap' },
    histTpoPill: { fontSize: '12px', fontWeight: '800', color: 'white', background: 'rgba(255,255,255,0.25)', padding: '4px 12px', borderRadius: '20px' },
    histRetryPill: { fontSize: '10px', color: 'rgba(255,255,255,0.9)', background: 'rgba(255,255,255,0.18)', padding: '3px 9px', borderRadius: '20px' },
    histCreatedAt: { fontSize: '11px', color: 'rgba(255,255,255,0.7)', marginLeft: 'auto' },
    histHeaderBottom: { display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' },
    histDateBig: { fontSize: '17px', fontWeight: '800', color: 'white', letterSpacing: '-0.3px' },
    histWeatherPill: { fontSize: '11px', color: 'rgba(255,255,255,0.9)', background: 'rgba(255,255,255,0.18)', padding: '4px 10px', borderRadius: '20px' },
    histBody: { padding: '14px 16px 16px' },
    histGlobalDesc: { fontSize: '12px', color: theme.colors.textSub, lineHeight: '1.65', margin: '0 0 12px', fontStyle: 'italic', paddingLeft: '10px', borderLeft: `2px solid ${theme.colors.primaryLight}` },
    histOutfitBlock: { borderRadius: '14px', padding: '12px 14px', marginBottom: '10px', transition: 'all 0.15s' },
    histBlockHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '11px' },
    histBlockHeadLeft: { display: 'flex', alignItems: 'center', gap: '8px' },
    histOutfitNum: { width: '24px', height: '24px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '900', color: 'white', flexShrink: 0 },
    histStyleTag: { fontSize: '12px', fontWeight: '700', padding: '3px 10px', borderRadius: '20px' },
    histAcceptedBadge: { fontSize: '11px', fontWeight: '700', color: theme.colors.primary, background: 'white', padding: '4px 12px', borderRadius: '20px', border: `1.5px solid ${theme.colors.primary}` },
    histStrip: { display: 'flex', gap: '10px', flexWrap: 'wrap' },
    histStripItem: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', width: '70px' },
    histStripImg: { width: '70px', height: '70px', objectFit: 'cover', borderRadius: '12px', border: '2px solid rgba(255,255,255,0.9)', boxShadow: '0 2px 8px rgba(0,0,0,0.10)' },
    histStripImgEmpty: { width: '70px', height: '70px', borderRadius: '12px', background: theme.colors.primaryLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' },
    histCatLabel: { fontSize: '9px', fontWeight: '700', color: 'white', padding: '2px 6px', borderRadius: '5px' },
    histItemType: { fontSize: '10px', color: theme.colors.text, fontWeight: '500', margin: 0, textAlign: 'center', maxWidth: '70px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
    histNoItem: { fontSize: '11px', color: theme.colors.textLight, margin: 0, padding: '6px 0' },
    histChangeCta: { marginTop: '12px', width: '100%', padding: '10px', background: 'white', border: `1.5px solid ${theme.colors.primary}`, borderRadius: '10px', fontSize: '13px', fontWeight: '700', color: theme.colors.primary, cursor: 'pointer', letterSpacing: '-0.2px' },
    histEmpty: { fontSize: '12px', color: theme.colors.textLight, textAlign: 'center', padding: '16px 0' },
};

const CS: Record<string, React.CSSProperties> = {
    testTriggerBtn: { padding: '4px 12px', backgroundColor: theme.colors.primary, color: 'white', border: 'none', borderRadius: theme.radius.full, fontSize: '12px', cursor: 'pointer', fontWeight: '500' },
    inlineTestBox: { marginTop: '16px', backgroundColor: theme.colors.primaryLight, borderRadius: theme.radius.lg, padding: '20px', border: `1px solid ${theme.colors.primary}44` },
    ...
    resultBox: { backgroundColor: theme.colors.background, borderRadius: theme.radius.lg, padding: '20px', textAlign: 'center', marginTop: '16px' },
    ...
    saveColorBtn: { flex: 1, padding: '10px', backgroundColor: theme.colors.primary, color: 'white', border: 'none', borderRadius: theme.radius.full, fontSize: '14px', cursor: 'pointer', fontWeight: '600' },
    retryBtn: { flex: 1, padding: '10px', backgroundColor: theme.colors.background, color: theme.colors.textSub, border: 'none', borderRadius: theme.radius.full, fontSize: '14px', cursor: 'pointer' },
    typeBox: { backgroundColor: theme.colors.primaryLight, borderRadius: theme.radius.lg, padding: '16px', marginBottom: '20px' },
    ...
    paletteSectionTitle: { fontSize: '14px', fontWeight: '600', color: theme.colors.textSub, margin: '0 0 12px', paddingBottom: '6px', borderBottom: `1px solid ${theme.colors.border}` },
    paletteCard: { backgroundColor: theme.colors.background, borderRadius: theme.radius.md, padding: '14px', marginBottom: '10px', border: `1px solid ${theme.colors.border}` },
    ...
    uploadBtn: { padding: '10px 20px', backgroundColor: theme.colors.primary, color: 'white', border: 'none', borderRadius: theme.radius.full, fontSize: '14px', cursor: 'pointer', marginBottom: '16px', fontWeight: '500' },
