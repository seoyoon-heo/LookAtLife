import React, { useEffect, useState } from 'react';
  ...
  function Home() {
      ...
      const fetchRecommendation = async (tpo: string = '일상', eventId: number | null = null) => {
          setLoading(true);
          setShowEventRecommendPrompt(false);
          try {
              const body: { tpo: string; mode: string; linkedEvents: number[] } = { tpo, mode: 'rag', linkedEvents: [] };
      ...
      const getEventsOnDay = (date: Date): CalendarEvent[] => weekEvents.filter(e => {
          const d = new Date(e.eventDatetime);
                  ...
                  <div style={styles.outfitCard}>
                      ...
                      {recommendation ? (
                          <div style={styles.outfitContent}>
                              ...
                              {recommendation.matched_items && recommendation.matched_items.length > 0 && (
  ...
  const styles: Record<string, React.CSSProperties> = {
      ...
      weatherEmoji: {},
      sectionLabel: { fontSize: '17px', fontWeight: '600', color: theme.colors.text, margin: '0 0 12px' },
      outfitCard: {
          backgroundColor: theme.colors.white, borderRadius: theme.radius.xl,
          overflow: 'hidden', marginBottom: '24px', boxShadow: theme.colors.cardShadow
      },
      outfitCardHeader: { backgroundColor: theme.colors.primary, padding: '12px 20px' },
      outfitCardBadge: { fontSize: '15px', fontWeight: '600', color: theme.colors.white },
      outfitContent: { padding: '16px 20px' },
      outfitEmpty: { padding: '24px 20px', textAlign: 'center' },
      outfitEmptyTitle: { fontSize: '16px', fontWeight: '600', color: theme.colors.text, margin: '0 0 6px' },
      outfitEmptySub: { fontSize: '13px', color: theme.colors.textSub, margin: '0 0 16px', lineHeight: '1.5' },
      ...
      weekScroll: { display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '8px', scrollbarWidth: 'none' },
      weekCard: {
          minWidth: '110px', backgroundColor: theme.colors.white,
          borderRadius: theme.radius.lg, padding: '12px', cursor: 'pointer', flexShrink: 0
      },
      ...
      weekCardEvent: { fontSize: '11px', color: theme.colors.textSub, margin: '0 0 8px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' },
      weekCardImg: { width: '100%', height: '80px', objectFit: 'cover', borderRadius: theme.radius.md, marginTop: '4px' },
      ...
      popupOutfitCard: { backgroundColor: theme.colors.primaryLight, borderRadius: theme.radius.md, padding: '14px', marginBottom: '10px' },
